import os
import math # <-- IMPORTAÇÃO CORRETA E NECESSÁRIA
...
def calcular_detalhes_emprestimo(emprestimo: Dict[str, Any], data_referencia_str: str) -> Dict[str, Any]:
    ...
    if not emprestimo.get('data_ultima_parcela'): return {"error": f"Empréstimo ID {emprestimo.get('id')} sem data final."}
    ...
    if data_referencia.day >= primeira_parcela.day: parcelas_pagas += 1
    ...
    return {"primeira_parcela": primeira_parcela.strftime('%d/%m/%Y'), "parcelas_pagas": parcelas_pagas, "a_vencer": parcelas_a_vencer, "valor_ja_pago": round(valor_ja_pago, 2), "valor_a_vencer": round(valor_a_vencer, 2), "proxima_parcela": proxima_parcela_data.strftime('%d/%m/%Y') if parcelas_a_vencer > 0 else "Quitado"}

def calcular_proposta_renegociacao(valor_a_vencer: float, parcelas_a_vencer: int) -> Dict[str, Any]:
    percentual_desconto = 0.0
    if 1 <= parcelas_a_vencer <= 20: percentual_desconto = 0.35
    elif 21 <= parcelas_a_vencer <= 40: percentual_desconto = 0.40
    elif 41 <= parcelas_a_vencer <= 60: percentual_desconto = 0.45
    elif 61 <= parcelas_a_vencer <= 80: percentual_desconto = 0.50
    elif 81 <= parcelas_a_vencer <= 100: percentual_desconto = 0.55
    elif 101 <= parcelas_a_vencer <= 120: percentual_desconto = 0.60
    valor_desconto = valor_a_vencer * percentual_desconto
    valor_final_com_desconto = valor_a_vencer - valor_desconto
    return {"saldo_devedor_original": round(valor_a_vencer, 2), "parcelas_restantes": parcelas_a_vencer, "percentual_desconto_aplicado": int(percentual_desconto * 100), "valor_do_desconto": round(valor_desconto, 2), "valor_final_para_quitacao": round(valor_final_com_desconto, 2)}

def calcular_parcelamento_price(valor_total: float, taxa_juros_mensal: float, num_parcelas: int) -> float:
    if num_parcelas <= 0: return 0
    if taxa_juros_mensal == 0:
        valor_parcela = valor_total / num_parcelas
    else:
        i, n = taxa_juros_mensal, num_parcelas
        valor_parcela = valor_total * (i * ((1 + i)**n)) / (((1 + i)**n) - 1)
    return math.ceil(valor_parcela) # ARREDONDAMENTO CORRIGIDO
...
def analisar_cliente(cliente_id: str):
    try:
        ...
        resultados = [ {**e, **calcular_detalhes_emprestimo(e, data_ref)} for e in emprestimos ]
        resumo = {
...
def renegociar_multiplos_emprestimos():
    ...
    if not loan_ids: return jsonify({"error": "Nenhum empréstimo selecionado."}), 400
    try:
        ...
        taxa_juros = 0.005
        propostas = [{"numero_parcelas": n, "valor_parcela": calcular_parcelamento_price(saldo_devedor_total, taxa_juros, n)} for n in range(3, 121, 3)]
        return jsonify({"saldo_devedor_total": round(saldo_devedor_total, 2), "taxa_juros_aplicada": "0.5% a.m.", "opcoes_parcelamento": propostas})
    except Exception as e:
